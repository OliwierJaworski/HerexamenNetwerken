...
void execution( int internet_socket )
{
    char buffer[1000];
    char expected[] = "OK";
    int HOEVEELBEWERKINGEN = rand()%10;
    printf("aantal bewerkingen %d\n",HOEVEELBEWERKINGEN);
     int BewerkingenDone=1;
///////////////blijf repeate tot server OK send
    while(strcmp(buffer,expected)!=0)
    {
        BewerkingenDone++;
        if(BewerkingenDone <= HOEVEELBEWERKINGEN)
        {
///////////////generate numbers
            int num1 = rand() % 100;
            int num2 = rand() % 100;
...
///////////////put numbers into string
            char RandomBewerking[100];
            snprintf(RandomBewerking, sizeof(RandomBewerking), "%d,%c,%d", num1, operation, num2);
            printf("randombewerking:%d,%c,%d\n", num1, operation, num2);
///////////////send numbers to server
            int number_of_bytes_send = 0;
            number_of_bytes_send = send(internet_socket, RandomBewerking, strlen(RandomBewerking), 0);
...
///////////////listen for answer
        ...
        {
///////////////server stuurt nog 1 antwoord door en gaat weer luisteren
            /*int number_of_bytes_received = 0;
            number_of_bytes_received = recv(internet_socket, buffer, (sizeof buffer) - 1, 0);
            if (number_of_bytes_received == -1) {
                perror("recv");
...
///////////////bericht om connection af te sluiten
            int number_of_bytes_send = 0;
...
///////////////last message to end connection
    int number_of_bytes_send = 0;
