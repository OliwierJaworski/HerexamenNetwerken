...
void EstablishConnection( int internet_socket );
int main( int argc, char * argv[] )
{
    ...
    //Execution//
    /////////////
    EstablishConnection( internet_socket );
...
void EstablishConnection( int internet_socket )
{
    char StartRequest[3] ="GO";
    char buffereddata[3];
    memset(buffereddata, 0, sizeof(buffereddata));

    while(strncmp(StartRequest, buffereddata, strlen(StartRequest)) != 0)
    {
        ...
        else
        {
            ...
            strcpy(buffereddata, buffer);
        }

        if (strncmp(StartRequest, buffer, strlen(StartRequest)) == 0) {
            printf("Received GO message, Starting Server Process\n");
        }
    }
}
